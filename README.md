...
The original VBA script is effective because it came about fairly naturally, and follows our thought process as we worked through the questions asked of us by Steve and his parents. A disadvantage of the original VBA script is that is fairly lengthy and broken out into a number of different macros. And because we have a nested for loop, the code within runs many, many times, and means that the code takes longer to run and output our results. 

On the other hand, the refactored script does not have a nested "for" loop and therefore runs much faster, and is also easy to follow the thought process for. Because it runs faster, it will work more effectively on larger data sets. In this case, it was worth refactoring this code, as it did not take a huge amount of time and resulted in a significantly more efficient code.
